...
var positionInRangeTests = []struct {
	name     string
	pos      token.Position
...
func TestPositionInRange(t *testing.T) {
	for _, tt := range positionInRangeTests {
...
func TestGetSymbolName(t *testing.T) {
	for _, tt := range getSymbolNameTests {
		t.Run(tt.name, func(t *testing.T) {
...
// testProgram represents a simple program for testing: var x: Integer = 42;
...
var findNodeAtPositionTests = []struct {
	name         string
	line         int
	col          int
	expectFound  bool
...
func TestFindNodeAtPosition(t *testing.T) {
	for _, tt := range findNodeAtPositionTests {
		t.Run(tt.name, func(t *testing.T) {
			node := FindNodeAtPosition(testProgram, tt.line, tt.col)
